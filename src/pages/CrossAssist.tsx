...
const suggestedQuestions = [
  ...
  return  (
    <div className="flex flex-col gap-8 ">
      ...
      {/* Popular Questions (Horizontal with custom scrollbar) */}
      <Card className="p-6">
        <h3 className="text-lg font-bold mb-4 text-foreground">
          Popular Questions
        </h3>
        <div className="flex overflow-x-auto max-w-full space-x-3 pb-2 px-1 custom-scrollbar">
          {suggestedQuestions.map((question, index) => (
            <button
              key={index}
              onClick={() => handleSuggestedQuestion(question)}
              className="flex-shrink-0 whitespace-nowrap text-left px-4 py-2 rounded-lg border border-border/50 hover:border-primary/50 hover:bg-cv-blue-light/30 transition-all duration-200 text-sm"
            >
              {question}
            </button>
          ))}
